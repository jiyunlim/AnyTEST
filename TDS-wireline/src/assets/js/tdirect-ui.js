...
UI.sellerFront = function () {
	...
	$stickyBar.on('click', '.btn-toggle', function (e) {
      ...
      $layerBody.parents('.wireline-purchase-content').children().not('.sticky-bar').removeAttr('aria-hidden');
			$btnToggleText.text('적용 가입조건 상세보기 열기');
		} else {
			$layerBody.addClass('on');
      $layerBody.children('.btn-toggle').attr('aria-expanded', true);
      $layerBody.parents('.wireline-purchase-content').children().not('.sticky-bar').attr('aria-hidden','true');
